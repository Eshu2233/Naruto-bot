...
(async () => {
	const { data: lastCommit } = await axios.get('https://api.github.com/repos/Eshu2233/Naruto-bot/commits/main');
	...
	const { data: versions } = await axios.get('https://raw.githubusercontent.com/Eshu2233/Naruto-bot/main/versions.json');
	...
	for (const filePath in files) {
		...
		let getFile;
		try {
			const response = await axios.get(`https://raw.githubusercontent.com/Eshu2233/Naruto-bot/main/${filePath}`, {
	...
	try {
		const { data: rawPackageJSON } = await axios.get("https://raw.githubusercontent.com/Eshu2233/Naruto-bot/main/package.json");
		fs.writeFileSync(`${process.cwd()}/package.json`, JSON.stringify(typeof rawPackageJSON === 'object' ? rawPackageJSON : JSON.parse(rawPackageJSON), null, 2));
	} catch (e) {
		log.warn("UPDATE", "Failed to update package.json from raw source");
	}
